...
final class ClaudeHookBridge: ObservableObject {
    ...
    private nonisolated static func serve(_ connection: HookConnection,
        ...
        guard let line = connection.readLine(), let request = ClaudeHookRequest.decode(line) else {
            return
        }
    ...
    private static func writeScript(to url: URL, socketPath: String) throws {
        ...
        #
        # Claude Code PreToolUse hook. Reads the tool-call request on stdin, asks
        # the running NotchFlow app over a Unix socket, and prints the user's
        # decision on stdout.
        #
